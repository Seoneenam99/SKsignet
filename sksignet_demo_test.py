...
def draw_bar(df):
    ...
    fig.add_vline(x=0.8, line_width=3, line_dash="dot", line_color="red")
    return(fig)
...
if (len(map_selected) != 0):
    ...
    if (option != 'Not Selected'):
        ...
        with tab3:
            ...
            max_inx = len(measure_pd_df)
            new_row = pd.DataFrame({'MEASUREMENTS' : 'Not Selected'}, index=[0])
            measure_pd_df = pd.concat([measure_pd_df, new_row], ignore_index=True)
              
            option2 = st.selectbox(
                 'Select Measurement?',
                 (measure_pd_df), index=max_inx)
            if (option2 != 'Not Selected'):
